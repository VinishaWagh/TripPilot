  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 glass-strong border-b border-border/50">
      <div className="flex items-center justify-between px-4 py-3 lg:px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            <h1 className="text-lg font-bold text-gradient">SkyPort AI</h1>
            <p className="text-xs text-muted-foreground">India Edition</p>
          </div>
        </div>

        {/* Navigation - Desktop */}
        <nav className="hidden lg:flex items-center gap-1">
          {['Flights', 'Airports', 'Weather', 'Alerts'].map((item) => (
            <Button
              key={item}
              variant="ghost"
              size="sm"
      ...
      <motion.div
        ...
        className={cn(
          "lg:hidden overflow-hidden border-t border-border/50",
